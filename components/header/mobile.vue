    ...
    >
      ...
      <section>
        <div
          ...
          :style="`transition-delay: ${0.25 + (pathKeyArray.findIndex((item) => item == key) * 0.05)}s`"
        ...
        >
          <button
            ...
            :class="accordionTarget == useLowcase(key) ? 'text-hw' : 'text-gray-500'"
            type="button"
          ...
          <ul
            ...
            :style="
              ...
              'height: 0' :
              `height: ${list.length > 0 ? 2  + (2.5 * list.length - 1) : ''}rem`
            "
          >
            <li
              class="first:pt-5 last:pb-5 pl-8 text-gray-600"
              :class="{'text-hw': route.path == `/${useLowcase(key)}/${child.path}`}"
              v-for="child in list"
            >
              <a
